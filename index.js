...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('Whangsap BOT', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Whangsap', 'yellow'))
    console.log(color('[~>>]'), color('Whatsapp Bot Started', 'green'))
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `what up y'all~, Whangsap BOT. Untuk mencari tahu command BOT, ketik ${prefix}menu`)
        })
	    }
	}
    })
