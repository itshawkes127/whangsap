...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        // [IDENTIFY]
        const ownerNumber = "629999999@c.us"
        ...
        case 'getpic':
            ...
            const useriq = getnomber.id.replace('@','') + '@c.us'
                try {
                    ...
                    aruga.sendFileFromUrl(from, jnck, `awok.jpg` , `Nih Sob...`)
                } catch {
    ...
    if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa digunakan didalam grup!', id)
            ...
            .catch(() => {
                aruga.reply(from, 'Hayoloh, Ada Yang Error!!', id)
            })
            break
    case 'dare':
    if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa digunakan didalam grup!', id)
     ...
     if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
            ...
            aruga.sendFileFromUrl(from, raya, 'citacita.mp3', id)
                .then(() => console.log('Success Sending Cita-Cita'))
              })
             .catch(() => {
         ...
         case 'kbbi':
            ...
            case 'blackpink':
                ...
                aruga.sendFileFromUrl(from, `https://arugaz.my.id/api/textpro/blackpink?text=`, `${bpk}.jpg`, `Nih Sob........`, id)
                break
                case 'glowtext':
                    if (args.length == 0) return aruga.reply(from, `kirim perintah ${prefix}logoff [nama]`, id)
                    aruga.reply(from, mess.wait, id)
                    const srhdah = body.slice(10)
                    aruga.sendFileFromUrl(from, `https://arugaz.my.id/api/textpro/firework?text=${srhdah}`, `${srhdah}.jpg`, `nehh ngab`, id)
                    break
            case 'logoff':
                if (args.length == 0) return aruga.reply(from, `kirim perintah ${prefix}logoff [nama]`, id)
                aruga.reply(from, mess.wait, id)
        ...
        case 'pornhub':
            ...
            if (argz.length >= 2) {
                aruga.reply(from, `Sabar brok, eug prosses dulu....`, id)
        ...
        case 'thunder':
            ...
            await aruga.sendFileFromUrl(from, `https://arugaz.my.id/api/textpro/thundertext?text=${thndr}`, 'thndr.jpg', '', id)
            break
        case 'tebakgambar':
...
case 'setdesc':
    ...
    const ganti = await aruga.setGroupDescription(descnya)
        ...
        break
case 'quotesen':
const qtos = await axios.get(`https://arugaz.my.id/api/random/text/quotes`).then(res => {
...
break
        ...
        case 'listsurah':
            try {
                ...
                .then((response) => {
                    let hehex = '╔══▰ ⌜ L I S T  S U R A H ⌟ \n'
                    for (let i = 0; i < response.data.data.length; i++) {
                        hehex += '╠➥ '
                        hehex += response.data.data[i].name.transliteration.id.toLowerCase() + '\n'
                            }
                        hehex += '╚═▰ ⌜ *H Y R O Z E L  B O T* ⌟ '
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case 'ytmp3':
            ...
            case 'distance':
                ...
                break
                case 'glitch':
                    if (args.length === 0) return aruga.reply(from, `Kirim perintah *${prefix}glitch [ |Teks1|Teks2 ]*, contoh *${prefix}glitch |Hyrozel|Bot*`, id)
                    argz = body.trim().split('|')
                    if (argz.length >= 2) {
                        aruga.reply(from, mess.wait, id)
                        ...
                        aruga.sendFileFromUrl(from, `https://arugaz.my.id/api/textpro/glitchtext?text1=${glitch1}&text2=${glitch2}`)
                    } else {
...
case 'ytsearch':
    ...
    case 'kusonime':
		...
		case 'trendingtwit':
            ...
            if (args.length == 0) return aruga.reply(from, `Untuk mencari lagu dari youtube\n\nPenggunaan: ${prefix}play judul lagu`, id)
            axios.get(`http://arugaz.my.id/api/media/ytaudio?url=${body.slice(7)}`)
            .then(async (res) => {
...
*Text : ${swrt}*`
        ...
        case 'artimimpi':
            ...
            try {
            const resp = await axios.get('https://arugaz.my.id/api/primbon/tafsirmimpi?mimpi=' + body.slice(10) )
            if (resp.data.error) return aruga.reply(from, resp.data.error, id)
